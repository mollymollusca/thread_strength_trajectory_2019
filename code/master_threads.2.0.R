#master_threads.2.0
# 1. Import data, plot
# 1/18/18 updated to include date of analysis 
# Next steps: 
# automatically choose peaks and valleys
# next step is to evaluate the peaks and valleys = QC
# save the deltas and summarize in datasheet
# figure out which is the newest peaks and valleys thing.... I think 
...
del_d <- del_d[del_d>=0.02]
d <- density(del_d)
plot(d)
hist(del_d, breaks=c(0,.025,.05,.075,.1,.125,.15,.175,.2,.225,.25,.275,.3))
i = which.max(d$y)
M2 = d$x[i]
M2
sysdate <- Sys.Date()
...
input <- read.csv(file = paste("1","summary.csv"), header=T, stringsAsFactors = FALSE)
summary <- rbind(input[,2:9], output)
write.csv(summary, file = paste("1","summary.csv"))

plot(data_1$load~data_1$time, pch=".", cex=1.5)
points(del$Pload~del$Ptime, pch=16, col="pink", cex = .5)
