...
wd_deltas <- "~/thread_strength_trajectory_2019/analysis_20190501/step_5/all_deltas_incl.byhand"

# Create the following working directories
directory <- "~/thread_strength_trajectory_2019/analysis_20190501/step_7"
...
for(j in 1:length(file_names)){
  tryCatch({
    ...
    if(length(del_d_all)>0 & !is.na(median_del)){
      ...
      pdf(file = paste(file_names[j],".pdf", sep = ""))
      par(mfrow = c(3,1))
      if(!is.na(d[1])){
        plot(d)
      ...
      plot(data_1$time,data_1$load, pch = '.',
           ylab = "Load",
           xlab = "Time")
      title(main = paste(j,": ",specimen_label, sep = ""), sub = file_names[j])
